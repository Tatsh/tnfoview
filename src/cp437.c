...
/* Map each IBM437 (CP437) byte value to its Unicode code point. */
static const uint16_t cp437_to_unicode_table[256] = {
    ...
    0x0018, 0x0019, 0x001c, 0x001b, 0x007f, 0x001d, 0x001e, 0x001f, 0x0020, 0x0021, 0x0022, 0x0023,
    ...
    0x0078, 0x0079, 0x007a, 0x007b, 0x007c, 0x007d, 0x007e, 0x001a, 0x00c7, 0x00fc, 0x00e9, 0x00e2,
    ...
    0x03a3, 0x03c3, 0x03bc, 0x03c4, 0x03a6, 0x0398, 0x03a9, 0x03b4, 0x221e, 0x03c6, 0x03b5, 0x2229,
...
char *cp437_to_utf8(const unsigned char *input, size_t length) {
    ...
    for (size_t i = 0; i < length; i++) {
        uint16_t code_point = cp437_to_unicode_table[input[i]];
        if (code_point < 0x80) {
            output[out++] = (char)code_point;
        } else if (code_point < 0x800) {
            output[out++] = (char)(0xc0 | (code_point >> 6));
            output[out++] = (char)(0x80 | (code_point & 0x3f));
        } else {
            output[out++] = (char)(0xe0 | (code_point >> 12));
            output[out++] = (char)(0x80 | ((code_point >> 6) & 0x3f));
            output[out++] = (char)(0x80 | (code_point & 0x3f));
        }
    }
    output[out] = '\0';
